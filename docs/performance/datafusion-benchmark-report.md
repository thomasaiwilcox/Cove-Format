...
Status: current local Criterion report plus public corpus contract
...
The committed manifest lives in `crates/cove-bench/benchmarks/public-corpus.json`. Generated outputs include COVE, Parquet, COVX/COVM where the converter emits them, a synthetic COVE-CACHE fixture, corpus lock metadata with digests, and benchmark reports with planning, scan, end-to-end, row, coverage, and cache metrics.
