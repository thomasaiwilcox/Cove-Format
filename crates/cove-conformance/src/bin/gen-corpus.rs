...
use cove_core::{
    artifact::{
        coveai::{
            ...
            VectorSpaceDescriptorV1, AI_COMPANION_ARTIFACT_KIND_CVV2, AI_POLICY_KIND_VISIBILITY,
    ...
    constants::{
        ...
        PrimaryProfile, SectionKind, StorageClass, ValueTag, AI_FEATURE_MODEL_INPUT_IDENTITY,
...
fn coveai_full_descriptor_fixture() -> Vec<u8> {
...
fn coveai_model_input_identity_fixture() -> Vec<u8> {
    let mut tables = AiDescriptorTablesV1::default();
    tables.payload_refs.push(AiPayloadRefEntryV1 {
        payload_ref: 1,
        storage_kind: AiStorageKindV1::SectionDecodedRelative as u8,
...
fn coveai_vector_binding_families_fixture() -> Vec<u8> {
    let mut tables = AiDescriptorTablesV1::default();
    tables.payload_refs.push(AiPayloadRefEntryV1 {
        payload_ref: 1,
        storage_kind: AiStorageKindV1::SectionDecodedRelative as u8,
...
fn covev_require_model_input_identity_reject_fixture(mut bytes: Vec<u8>) -> Vec<u8> {
...
fn covi_test_chain_digest() -> Vec<u8> {
    vec![0xCD; 32]
}
...
fn covi_row_range_artifact_with_delta_chain_digest(delta_chain_digest: Vec<u8>) -> Vec<u8> {
    covi_artifact_for_postings(
        vec![key_bytes(1)],
        vec![posting_spec(
            CoviPostingRepresentationV2::RowRangeList,
...
fn covi_object_path_tombstone_overlay_artifact() -> Vec<u8> {
    covi_artifact_for_postings(
...
fn object_path_key_bytes() -> Vec<u8> {
...
fn covi_key_and_entries_with_kinds(
    keys: &[Vec<u8>],
    posting_refs: &[u32],
    aggregate_refs: &[u32],
    duplicate_refs: &[u32],
...
fn feature_binding_empty_payload_contract_payload() -> Vec<u8> {
...
fn serialize_invalid_feature_binding_section(section: &SectionFeatureBindingSectionV2) -> Vec<u8> {
...
fn cove_o_object_catalog_section_from_catalog(catalog: ObjectTypeCatalog) -> SectionPayload {
    SectionPayload {
...
fn cove_o_temporal_segment_index_section_for_object_type(
    object_type_id: u32,
    segments: &[(u32, &[TemporalRowEntryV1])],
) -> SectionPayload {
...
fn cove_o_temporal_segment_data_section_for_object_type(
    segment_id: u32,
    object_type_id: u32,
    rows: &[TemporalRowEntryV1],
) -> SectionPayload {
...
fn cove_o_trust_manifest_filecode_reassignment_file() -> Vec<u8> {
...
fn delta_only_temporal_rows() -> Vec<TemporalRowEntryV1> {
...
fn duplicate_record_id_temporal_rows() -> Vec<TemporalRowEntryV1> {
    vec![
        TemporalRowEntryV1 {
            timestamp_us: 10,
            csn: 1,
...
fn valid_tombstone_temporal_rows() -> Vec<TemporalRowEntryV1> {
    vec![TemporalRowEntryV1 {
        timestamp_us: 10,
        csn: 1,
        branch_key: 0,
...
fn temporal_segment_data_payload_for_object_type(
    segment_id: u32,
    object_type_id: u32,
    rows: &[TemporalRowEntryV1],
) -> Vec<u8> {
...
fn temporal_segment_data_payload_with_filecode_property(
    segment_id: u32,
    rows: &[TemporalRowEntryV1],
    codes: &[u32],
) -> Vec<u8> {
...
fn trust_manifest_payload_for_segment_with_dictionary(
...
fn temporal_segment_entry_for_rows_object_type(
    segment_id: u32,
    object_type_id: u32,
    rows: &[TemporalRowEntryV1],
) -> TemporalSegmentIndexEntryV1 {
...
fn covm_delta_required_non_delta_reader_file() -> Vec<u8> {
...
fn valid_covedelta_file() -> Vec<u8> {
    covedelta_file_with_parent_refs(vec![valid_covedelta_lineage_parent_ref()])
}

fn covedelta_missing_lineage_parent_file() -> Vec<u8> {
    covedelta_file_with_parent_refs(Vec::new())
}

fn covedelta_duplicate_lineage_parent_file() -> Vec<u8> {
...
fn valid_covedelta_lineage_parent_ref() -> DeltaParentRefV1 {
    DeltaParentRefV1 {
        parent_ref: 0,
        parent_kind: 0,
...
fn covedelta_file_with_parent_refs(parent_refs: Vec<DeltaParentRefV1>) -> Vec<u8> {
    CoveDeltaFile {
...
fn valid_covedelta_object_delta_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows(&valid_temporal_rows())
}

fn covedelta_object_delta_file_with_rows(rows: &[TemporalRowEntryV1]) -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(rows, 0, Vec::new())
}

fn covedelta_object_delta_checkpoint_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
...
fn covedelta_object_delta_checkpoint_missing_rows_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
...
fn covedelta_object_delta_with_anchor_touched_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
...
fn covedelta_object_delta_with_touched_property_bitmap_file(
...
fn covedelta_object_delta_with_underinclusive_touched_file() -> Vec<u8> {
    let mut touched = valid_delta_touched_object_range();
...
fn covedelta_object_delta_with_underinclusive_anchor_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_unresolved_anchor_state_hash_file() -> Vec<u8> {
    let mut anchor = valid_delta_continuation_anchor();
...
fn covedelta_object_delta_with_single_scope_anchor_mismatch_file() -> Vec<u8> {
    let mut anchor = valid_delta_continuation_anchor();
...
fn covedelta_object_delta_with_parent_alias_dictionary_overlay_file() -> Vec<u8> {
    let mut entry = valid_delta_dictionary_entry();
...
fn covedelta_object_delta_with_parent_alias_dictionary_overlay_missing_feature_file() -> Vec<u8> {
    let mut entry = valid_delta_dictionary_entry();
    entry.entry_kind = DELTA_DICTIONARY_ENTRY_KIND_PARENT_DICTIONARY_ALIAS;
    entry.inline_value_ref = DELTA_REF_NONE;
    entry.parent_ref = 0;
...
fn covedelta_object_delta_with_parent_alias_dictionary_overlay_unknown_parent_ref_file() -> Vec<u8>
{
    let mut entry = valid_delta_dictionary_entry();
    entry.entry_kind = DELTA_DICTIONARY_ENTRY_KIND_PARENT_DICTIONARY_ALIAS;
    entry.inline_value_ref = DELTA_REF_NONE;
...
fn covedelta_object_delta_with_hash_hint_dictionary_overlay_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
        0,
...
fn covedelta_object_delta_with_zero_hash_hint_dictionary_overlay_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_required_hash_hint_dictionary_overlay_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_descriptor_tables_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
        0,
...
fn covedelta_object_delta_with_sparse_scope_descriptor_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_sparse_summary_descriptor_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_catalog_patch_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_projection_patch_file() -> Vec<u8> {
    let mut delta = CoveDeltaFile::parse(&covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
...
fn covedelta_object_delta_with_projection_patch_missing_section_file() -> Vec<u8> {
    let mut delta = CoveDeltaFile::parse(&covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
        DELTA_FEATURE_PROJECTION_PATCH,
        Vec::new(),
...
fn covedelta_object_delta_with_projection_patch_missing_fingerprint_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
        DELTA_FEATURE_PROJECTION_PATCH,
        vec![delta_projection_patch_section(
...
fn covedelta_object_delta_with_index_hints_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_index_hints_missing_section_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
        DELTA_FEATURE_INDEX_HINTS,
        Vec::new(),
...
fn covedelta_object_delta_with_index_hints_lineage_parent_file() -> Vec<u8> {
    covedelta_object_delta_with_sidecar_hint_file(
        CoveDeltaSectionKind::IndexHints,
        DELTA_FEATURE_INDEX_HINTS,
        DELTA_FEATURE_INDEX_HINTS,
...
fn covedelta_object_delta_with_coverage_patch_file() -> Vec<u8> {
    covedelta_object_delta_with_sidecar_hint_file(
        CoveDeltaSectionKind::CoveragePatch,
...
fn covedelta_object_delta_with_coverage_patch_wrong_kind_file() -> Vec<u8> {
    covedelta_object_delta_with_sidecar_hint_file(
        CoveDeltaSectionKind::CoveragePatch,
        DELTA_FEATURE_COVERAGE_PATCH,
        DELTA_FEATURE_COVERAGE_PATCH,
...
fn covedelta_with_layout_hints_file() -> Vec<u8> {
    covedelta_object_delta_with_sidecar_hint_file(
        CoveDeltaSectionKind::LayoutHints,
        0,
        0,
...
fn covedelta_with_layout_hints_wrong_kind_file() -> Vec<u8> {
    covedelta_object_delta_with_sidecar_hint_file(
        CoveDeltaSectionKind::LayoutHints,
        0,
        0,
...
fn covedelta_object_delta_with_sidecar_hint_file(
...
fn covedelta_object_delta_with_corrupt_optional_index_file() -> Vec<u8> {
    covedelta_object_delta_with_corrupt_optional_section_file(
        CoveDeltaSectionKind::IndexHints,
...
fn covedelta_object_delta_with_optional_index_unknown_required_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_corrupt_optional_section_file(
    section_kind: CoveDeltaSectionKind,
    payload: &[u8],
) -> Vec<u8> {
...
fn covedelta_object_delta_with_optional_section_file(
    section_kind: CoveDeltaSectionKind,
    payload: &[u8],
...
fn covedelta_object_delta_with_evidence_patch_missing_map_fingerprint_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
...
fn delta_property_ops_section(section_id: u32, payload: Vec<u8>) -> CoveDeltaSection {
    CoveDeltaSection {
        entry: CoveDeltaSectionDirectoryEntryV1 {
            section_id,
...
fn delta_dictionary_overlay_section(
    section_id: u32,
...
fn delta_descriptor_table_sections(
...
fn delta_scope_descriptor_section(
    section_id: u32,
...
fn delta_summary_descriptor_section(
    section_id: u32,
    section_kind: CoveDeltaSectionKind,
...
fn catalog_patch_temporal_rows() -> Vec<TemporalRowEntryV1> {
    vec![TemporalRowEntryV1 {
        timestamp_us: 20,
        csn: 2,
        branch_key: 0,
...
fn catalog_patch_new_widget_object_type() -> ObjectTypeCatalog {
    ObjectTypeCatalog {
        flags: 0,
        types: vec![ObjectTypeEntryV1 {
            object_type_id: 2,
...
fn catalog_patch_reinterprets_parent_property() -> ObjectTypeCatalog {
    ObjectTypeCatalog {
        flags: 0,
        types: vec![ObjectTypeEntryV1 {
            object_type_id: 1,
...
fn catalog_patch_section(section_id: u32, patch: ObjectTypeCatalog) -> CoveDeltaSection {
    CoveDeltaSection {
        entry: CoveDeltaSectionDirectoryEntryV1 {
            section_id,
...
fn covedelta_object_delta_with_tombstone_set_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_tombstone_temporal_rows(),
        DELTA_FEATURE_EXACT_TOMBSTONE_SET,
        vec![CoveDeltaSection {
...
fn covedelta_object_membership_scope_case() -> Vec<u8> {
...
fn covedelta_object_projection_property_skip_case() -> Vec<u8> {
    let matching_scope_id = [0; 16];
...
fn covedelta_covi_base_tombstone_overlay_case() -> Vec<u8> {
    let tombstoned_goid = [0x09; 16];
...
fn covedelta_tombstone_reconstruction_case() -> Vec<u8> {
    let tombstoned_goid = [0x09; 16];
...
fn covedelta_association_update_reconstruction_case() -> Vec<u8> {
...
fn covedelta_evidence_patch_inherited_map_fingerprint_case() -> Vec<u8> {
...
fn covedelta_projection_patch_inherited_projection_fingerprint_case() -> Vec<u8> {
    let inherited_projection_fingerprint_ref = 43u32;
    let patched_projection_fingerprint_ref = 44u32;
...
fn delta_map_evidence_patch_payload() -> Vec<u8> {
...
fn delta_map_evidence_patch_section(section_id: u32, payload: Vec<u8>) -> CoveDeltaSection {
    CoveDeltaSection {
        entry: CoveDeltaSectionDirectoryEntryV1 {
            section_id,
...
fn delta_projection_patch_payload() -> Vec<u8> {
    map_payload_bytes(covemap_payload_value(
...
fn delta_sidecar_parent_ref(parent_ref: u32) -> DeltaParentRefV1 {
    DeltaParentRefV1 {
        parent_ref,
        parent_kind: 0,
        flags: 0,
...
fn delta_sidecar_hint(parent_ref: u32, hint_kind: u16) -> DeltaSidecarHintV1 {
    DeltaSidecarHintV1 {
        hint_ref: 0,
        hint_kind,
        flags: 0,
...
fn association_object_catalog(association_object_type_id: u32) -> ObjectTypeCatalog {
    ObjectTypeCatalog {
        flags: 0,
        types: vec![ObjectTypeEntryV1 {
            object_type_id: association_object_type_id,
...
fn covedelta_base_plus_one_delta_reconstruction_case() -> Vec<u8> {
...
fn covedelta_compaction_equivalence_case() -> Vec<u8> {
    let base_goid = [0x01; 16];
    let base_record_id = [0x02; 16];
    let delta_goid = [0x0D; 16];
    let delta_record_id = [0x0E; 16];
...
fn covedelta_catalog_patch_reconstruction_case() -> Vec<u8> {
    let base_goid = [0x01; 16];
    let base_record_id = [0x02; 16];
    let delta_goid = [0x0D; 16];
    let delta_record_id = [0x0E; 16];
...
fn covedelta_catalog_patch_reinterprets_parent_case() -> Vec<u8> {
    let base_rows = vec![TemporalRowEntryV1 {
        timestamp_us: 10,
        csn: 1,
        branch_key: 0,
...
fn covedelta_object_delta_with_touched_tombstone_sets_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_underinclusive_tombstone_file() -> Vec<u8> {
...
fn covedelta_object_delta_with_branch_identity_file() -> Vec<u8> {
    covedelta_object_delta_file_with_rows_and_sections(
        &valid_temporal_rows(),
        0,
        vec![CoveDeltaSection {
...
fn covedelta_object_delta_file_with_rows_and_sections(
    rows: &[TemporalRowEntryV1],
    required_delta_features: u64,
...
fn covedelta_object_delta_file_with_rows_object_type_and_sections(
    rows: &[TemporalRowEntryV1],
    object_type_id: u32,
    required_delta_features: u64,
...
fn rewrite_covedelta_required_features(bytes: &mut [u8], required_delta_features: u64) {
...
fn valid_delta_continuation_anchor() -> DeltaContinuationAnchorV1 {
...
fn valid_delta_state_hash_section(section_id: u32) -> CoveDeltaSection {
    CoveDeltaSection {
        entry: CoveDeltaSectionDirectoryEntryV1 {
            section_id,
...
fn valid_delta_state_hash_descriptor() -> DeltaStateHashDescriptorV1 {
...
fn covedelta_state_hash_recompute_case() -> Vec<u8> {
...
fn valid_delta_state_hash_material() -> CoveObjectDeltaStateHashV1 {
...
fn valid_delta_sparse_patch_record() -> DeltaSparsePatchRecordV1 {
...
fn valid_delta_dictionary_entry() -> DeltaDictionaryEntryV1 {
...
fn valid_delta_canonical_hash_hint_dictionary_entry() -> DeltaDictionaryEntryV1 {
    DeltaDictionaryEntryV1 {
...
fn valid_delta_scope_descriptor() -> DeltaScopeDescriptorV1 {
...
fn valid_delta_summary_descriptor(summary_kind: u8) -> DeltaSummaryDescriptorV1 {
...
fn covedelta_sparse_patch_state_reconstruction_case() -> Vec<u8> {
...
fn sparse_patch_record_missing_value_ref_bytes() -> Vec<u8> {
...
fn sparse_patch_record_null_with_payload_bytes() -> Vec<u8> {
    let mut bytes = valid_delta_sparse_patch_record().serialize().unwrap();
...
fn sparse_patch_record_bad_tombstone_kind_bytes() -> Vec<u8> {
    let mut bytes = valid_delta_sparse_patch_record().serialize().unwrap();
    let property_offset =
...
fn sparse_patch_record_missing_redaction_ref_bytes() -> Vec<u8> {
    let mut bytes = valid_delta_sparse_patch_record().serialize().unwrap();
    let property_offset =
...
fn sparse_patch_record_unsorted_properties_bytes() -> Vec<u8> {
    let mut bytes = valid_delta_sparse_patch_record().serialize().unwrap();
...
fn rewrite_sparse_patch_record_crc(bytes: &mut [u8]) {
...
fn valid_delta_branch_identity() -> DeltaBranchIdentityV1 {
...
fn valid_delta_touched_object_range() -> DeltaTouchedObjectRangeV1 {
...
fn valid_delta_tombstone_object_range() -> DeltaTouchedObjectRangeV1 {
    DeltaTouchedObjectRangeV1 {
        scope_kind: 0,
        scope_id: [0; 16],
        object_type_id: 1,
...
fn valid_covm_delta_chain_extension() -> Vec<u8> {
    valid_covm_delta_chain_extension_model()
        .serialize()
        .unwrap()
}

fn covm_delta_chain_extension_duplicate_artifact_id() -> Vec<u8> {
...
fn valid_covm_delta_chain_extension_model() -> CovmDeltaChainExtensionV1 {
    let base_snapshot_id = [0x91; 16];
...
fn valid_covm_delta_chain_summary() -> Vec<u8> {
...
fn covm_delta_chain_selection_case(corrupt_delta_digest: bool) -> Vec<u8> {
    covm_delta_chain_selection_case_with_summary_mutation(corrupt_delta_digest, |_| {})
}

fn covm_delta_chain_selection_delta_count_case(delta_count: usize) -> Vec<u8> {
...
fn covm_delta_chain_selection_summary_underinclude_csn_case() -> Vec<u8> {
    covm_delta_chain_selection_case_with_summary_mutation(false, |entry| {
        entry.csn_min = entry.csn_min.saturating_add(1);
    })
}

fn covm_delta_chain_selection_summary_underinclude_commit_case() -> Vec<u8> {
    covm_delta_chain_selection_case_with_summary_mutation(false, |entry| {
        entry.commit_time_end_us -= 1;
    })
}

fn covm_delta_chain_selection_wrong_parent_snapshot_case() -> Vec<u8> {
...
fn covm_delta_chain_selection_with_base_case(stale_lineage_parent: bool) -> Vec<u8> {
...
fn covm_delta_chain_selection_reordered_deltas_case() -> Vec<u8> {
    let dataset_id = [0x81; 16];
    let base_snapshot_id = [0x82; 16];
...
fn covm_delta_chain_selection_required_summary_case(
...
fn covm_delta_chain_selection_summary_wrong_chain_digest_case() -> Vec<u8> {
    let mut delta = CoveDeltaFile::parse(&valid_covedelta_object_delta_file()).unwrap();
    let base_snapshot_id = delta.header.parent_snapshot_id;
    let result_snapshot_id = delta.header.snapshot_id;
    let dataset_id = delta.header.dataset_id;
...
fn covm_delta_chain_selection_required_summary_digest_mismatch_case() -> Vec<u8> {
...
fn covm_delta_chain_selection_unadvertised_required_delta_feature_case() -> Vec<u8> {
...
fn covm_delta_chain_selection_case_with_summary_mutation(
...
fn selected_chain_covedelta_file(
    artifact_id: [u8; 16],
    dataset_id: [u8; 16],
    snapshot_id: [u8; 16],
...
fn align_covedelta_lineage_parent_with_covm_ref(
...
fn covm_base_artifact_ref_from_bytes(
    base_bytes: &[u8],
    artifact_id: [u8; 16],
    snapshot_id: [u8; 16],
...
fn covm_delta_artifact_ref_from_delta_bytes(delta_bytes: &[u8]) -> CovmDeltaArtifactRefV1 {
    let delta = CoveDeltaFile::parse(delta_bytes).unwrap();
    let digest = compute_digest(DigestAlgorithm::Sha256, delta_bytes).unwrap();
    let mut digest_array = [0u8; 32];
    digest_array.copy_from_slice(&digest);
...
fn covm_delta_pruning_case_with_source_range(
    as_of_csn: Option<u64>,
    as_of_commit_timestamp_us: Option<i64>,
...
fn covm_delta_pruning_valid_time_no_summary_case() -> Vec<u8> {
    let mut value: serde_json::Value =
...
fn valid_covm_delta_pruning_summary(missing_commit_fields: bool) -> Vec<u8> {
    let mut entries = vec![
...
fn covm_delta_pruning_summary_entry(
...
fn covm_delta_summary_entry(reference: CovmDeltaArtifactRefV1) -> DeltaChainSummaryEntryV1 {
    DeltaChainSummaryEntryV1 {
...
fn rewrite_covm_delta_chain_required_features(bytes: &mut [u8], required_delta_features: u64) {
    bytes[8..16].copy_from_slice(&required_delta_features.to_le_bytes());
    rewrite_covm_delta_chain_header_crc(bytes);
}

fn rewrite_covm_delta_ref_ordinal(bytes: &mut [u8], ref_index: usize, chain_ordinal: u32) {
...
fn rewrite_covm_delta_ref_artifact_id(bytes: &mut [u8], ref_index: usize, artifact_id: [u8; 16]) {
    let offset = COVM_DELTA_CHAIN_EXTENSION_HEADER_LEN + ref_index * COVM_DELTA_ARTIFACT_REF_LEN;
...
fn rewrite_covm_delta_chain_header_crc(bytes: &mut [u8]) {
...
fn rewrite_covm_delta_summary_entry_ordinal(
    bytes: &mut [u8],
    entry_index: usize,
    chain_ordinal: u32,
) {
...
fn rewrite_covm_delta_summary_entry_required_features(
    bytes: &mut [u8],
    entry_index: usize,
    required_delta_features: u64,
) {
...
fn rewrite_covm_delta_summary_entry_csn_min(bytes: &mut [u8], entry_index: usize, csn_min: u64) {
    let offset = COVM_DELTA_CHAIN_SUMMARY_HEADER_LEN as usize
        + 32
        + entry_index * COVM_DELTA_CHAIN_SUMMARY_ENTRY_LEN;
...
fn rewrite_covm_delta_summary_entry_commit_start(
    bytes: &mut [u8],
    entry_index: usize,
    commit_time_start_us: i64,
) {
...
fn rewrite_covm_delta_summary_entry_crc(bytes: &mut [u8], offset: usize) {
...
fn rewrite_covm_delta_chain_summary_header_crc(bytes: &mut [u8]) {
