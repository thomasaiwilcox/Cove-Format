...
fn validate_covm_delta_chain_selection_fixture(bytes: &[u8]) -> Result<(), CoveError> {
...
fn validate_covm_delta_pruning_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
...
fn validate_covedelta_sparse_patch_state_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
        CoveError::BadSection(format!(
...
fn validate_covedelta_object_membership_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
        CoveError::BadSection(format!(
...
fn validate_covedelta_covi_tombstone_overlay_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
        CoveError::BadSection(format!(
...
fn validate_covedelta_reconstruction_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
        CoveError::BadSection(format!(
...
fn validate_covedelta_state_hash_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
        CoveError::BadSection(format!(
...
fn validate_zero_copy_compat_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
...
fn validate_coverage_proof_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
...
fn validate_sidecar_validity_fixture(bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
...
fn covi_validation_context_from_fixture_value(
    context_value: &Value,
...
fn json_byte_vec(value: &Value, field: &str) -> Result<Vec<u8>, CoveError> {
...
fn validate_runtime_operation_fixture(bytes: &[u8]) -> Result<(), CoveError> {
...
fn json_field_str<'a>(value: &'a Value, field: &str) -> Result<&'a str, CoveError> {
    value
        .get(field)
        .and_then(Value::as_str)
...
fn json_field_u16(value: &Value, field: &str) -> Result<u16, CoveError> {
    let raw = value
        .get(field)
...
fn json_field_u32(value: &Value, field: &str) -> Result<u32, CoveError> {
    let raw = value
        .get(field)
        .ok_or_else(|| CoveError::BadSection(format!("missing integer field {field}")))?;
    json_u32(raw, field)
...
fn json_field_u8(value: &Value, field: &str) -> Result<u8, CoveError> {
    let raw = value
        .get(field)
        .ok_or_else(|| CoveError::BadSection(format!("missing integer field {field}")))?;
    raw.as_u64()
...
fn json_field_bool(value: &Value, field: &str) -> Result<bool, CoveError> {
    value
        .get(field)
        .and_then(Value::as_bool)
...
fn json_u32(value: &Value, field: &str) -> Result<u32, CoveError> {
    value
        .as_u64()
...
fn optional_u32_array(value: &Value, field: &str) -> Result<Option<Vec<u32>>, CoveError> {
...
fn validate_cove_map_candidates_fixture(corpus: &Path, bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
...
fn validate_cove_map_replay_fixture(corpus: &Path, bytes: &[u8]) -> Result<(), CoveError> {
    let value: Value = serde_json::from_slice(bytes).map_err(|error| {
