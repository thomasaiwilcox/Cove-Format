...
pub enum CoveError {
    ...
    MapEvidenceInvalid,
    /// COVE-CX codec descriptor, envelope, payload, fallback, or feature-bit contract is invalid.
    BadCodecExtension,
    /// Required registered codec is unsupported and no valid fallback exists.
    CodecUnsupported,
    /// COVE-L layout, split, cluster, zero-copy, or metadata planning record is invalid.
    BadLayoutPlan,
    /// Required COVE-R runtime compatibility hint is unsupported for the requested operation.
...
impl fmt::Display for CoveError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CoveError::BadCodecExtension => write!(
                f,
                "COVE_E_BAD_CODEC_EXTENSION: codec descriptor, envelope, fallback, or feature contract invalid"
            ),
            CoveError::CodecUnsupported => write!(
                f,
                "COVE_E_CODEC_UNSUPPORTED: required registered codec unsupported without valid fallback"
            ),
            CoveError::BadLayoutPlan => write!(
                f,
                "COVE_E_BAD_LAYOUT_PLAN: layout plan, split index, zero-copy map, or fast metadata invalid"
            ),
            CoveError::RuntimeHintUnsupported => write!(
                f,
                "COVE_E_RUNTIME_HINT_UNSUPPORTED: required runtime hint unsupported for operation"
            ),
            CoveError::BadCoverage => write!(
                f,
...
impl CoveError {
    ...
    pub fn spec_code(&self) -> Option<&'static str> {
        match self {
            ...
            CoveError::BadCodecExtension => Some("COVE_E_BAD_CODEC_EXTENSION"),
            CoveError::CodecUnsupported => Some("COVE_E_CODEC_UNSUPPORTED"),
...
impl PartialEq for CoveError {
    fn eq(&self, other: &Self) -> bool {
        ...
        match (self, other) {
            ...
            | (BadCodecExtension, BadCodecExtension)
