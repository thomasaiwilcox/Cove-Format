...
use crate::{
    checksum,
    constants::{
        CompressionCodec, KNOWN_FEATURE_BITS_MASK, MAGIC_COVE, POSTSCRIPT_LEN,
...
/// Size in bytes of the serialised [`CoveSectionSpecV1`].
...
///
/// Corresponds to `CoveSectionSpecV1` in Section 12 of the specification.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CoveSectionSpecV1 {
...
///
/// Corresponds to `CovePostscriptV1` in Section 12 of the specification.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CovePostscriptV1 {
...
impl CovePostscriptV1 {
    ...
    pub fn serialize_tail(&self) -> [u8; POSTSCRIPT_TOTAL_SIZE] {
        ...
        tail[POSTSCRIPT_SIZE + 4..POSTSCRIPT_SIZE + 8].copy_from_slice(&MAGIC_COVE);
        tail
    }
...
mod tests {
    ...
    fn minimal_spec() -> CoveSectionSpecV1 {
        CoveSectionSpecV1 {
            offset: 128,
            length: 44,
    ...
    fn minimal_postscript(file_len: u64) -> CovePostscriptV1 {
        CovePostscriptV1 {
            required_features: 0,
            optional_features: 0,
            file_len,
    ...
    fn section_spec_roundtrip() {
        ...
        let parsed = CoveSectionSpecV1::parse(&bytes).expect("parse should succeed");
    ...
    fn postscript_tail_roundtrip() {
        // A minimal COVE file: header (128) + footer (44) + postscript tail (72)
        // Total: 244 bytes
        ...
        let parsed =
            CovePostscriptV1::parse_from_tail(&file).expect("parse_from_tail should succeed");
    ...
    fn section_spec_end_offset_overflow_rejected() {
        ...
        assert_eq!(spec.end_offset(), Err(CoveError::ArithOverflow));
    }
    ...
    fn bad_postscript_version_rejected() {
        ...
        tail[POSTSCRIPT_SIZE..POSTSCRIPT_SIZE + 2].copy_from_slice(&1u16.to_le_bytes());
        let mut file = vec![0u8; file_len as usize];
        file[file_len as usize - POSTSCRIPT_TOTAL_SIZE..].copy_from_slice(&tail);
        assert_eq!(
            CovePostscriptV1::parse_from_tail(&file),
