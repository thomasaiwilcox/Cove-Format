...
/// Flag bit: domain belongs to a COVE-O object/property pair (cleared = COVE-T
...
impl ColumnDomain {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, CoveError> {
        ...
        let sfc_off =
            usize::try_from(header.sorted_file_codes_offset).map_err(|_| CoveError::OffsetRange)?;
        ...
        let rank_off =
            usize::try_from(header.file_code_to_rank_offset).map_err(|_| CoveError::OffsetRange)?;
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, CoveError> {
        let domain_count =
            u32::try_from(self.sorted_file_codes.len()).map_err(|_| CoveError::BadDomain)?;
    ...
    /// Validate `is_safe` and return [`CoveError::BadDomain`] otherwise.
...
mod tests {
    ...
    fn detects_unsafe_rank_map() {
        ...
        assert!(!d.is_safe());
        assert_eq!(d.validate(), Err(CoveError::BadDomain));
    }
    ...
    fn from_sorted_rejects_out_of_range_codes() {
        ...
        assert_eq!(err, CoveError::BadFileCode);
    }
