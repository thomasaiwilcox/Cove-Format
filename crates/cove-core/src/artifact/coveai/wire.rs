...
pub(super) fn read_u8(bytes: &[u8], offset: usize) -> Result<u8, CoveError> {
    bytes.get(offset).copied().ok_or(CoveError::BufferTooShort)
}

pub(super) fn read_u16(bytes: &[u8], offset: usize) -> Result<u16, CoveError> {
...
pub(super) fn read_u32(bytes: &[u8], offset: usize) -> Result<u32, CoveError> {
    let end = offset.checked_add(4).ok_or(CoveError::ArithOverflow)?;
    let slice = bytes.get(offset..end).ok_or(CoveError::BufferTooShort)?;
    Ok(u32::from_le_bytes(slice.try_into().unwrap()))
}

pub(super) fn read_u64(bytes: &[u8], offset: usize) -> Result<u64, CoveError> {
    let end = offset.checked_add(8).ok_or(CoveError::ArithOverflow)?;
    let slice = bytes.get(offset..end).ok_or(CoveError::BufferTooShort)?;
    Ok(u64::from_le_bytes(slice.try_into().unwrap()))
}

pub(super) fn read_i64(bytes: &[u8], offset: usize) -> Result<i64, CoveError> {
    let end = offset.checked_add(8).ok_or(CoveError::ArithOverflow)?;
    let slice = bytes.get(offset..end).ok_or(CoveError::BufferTooShort)?;
