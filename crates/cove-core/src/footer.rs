...
//!
//! 1. A fixed 44-byte `CoveFooterHeaderV1`.
//! 2. An array of `section_count` × 76-byte `CoveSectionEntryV1` records.
...
/// Fixed 44-byte header at the start of every COVE footer.
///
...
impl CoveFooterHeaderV1 {
    ...
    pub fn total_len(&self) -> Result<u64, CoveError> {
...
///
/// Corresponds to `CoveSectionEntryV1` in Section 13 of the specification.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CoveSectionEntryV1 {
...
/// Parsed COVE footer: header + section directory + optional JSON metadata.
#[derive(Debug, Clone)]
pub struct CoveFooter {
    /// The fixed footer header.
    pub header: CoveFooterHeaderV1,
    /// The section directory.
    pub sections: Vec<CoveSectionEntryV1>,
...
impl CoveFooter {
    ...
    pub fn parse(buf: &[u8]) -> Result<Self, CoveError> {
        let header = CoveFooterHeaderV1::parse(buf)?;
        ...
        for i in 0..header.section_count as usize {
            ...
            let entry = CoveSectionEntryV1::parse(&buf[start..])?;
            sections.push(entry);
        }
    ...
    pub fn metadata(&self) -> Result<MetadataJson, CoveError> {
...
mod tests {
    ...
    fn empty_footer_roundtrip() {
        ...
        let parsed = CoveFooter::parse(&bytes).expect("parse should succeed");
    ...
    fn section_entry_roundtrip() {
        ...
        let parsed = CoveSectionEntryV1::parse(&bytes).expect("parse should succeed");
    ...
    fn section_entry_encryption_nonzero_rejected() {
        ...
        bytes[49] = 1;
        assert!(matches!(
            CoveSectionEntryV1::parse(&bytes),
    ...
    fn section_entry_end_offset_overflow_rejected() {
        ...
        assert_eq!(entry.end_offset(), Err(CoveError::ArithOverflow));
    }
    ...
    fn footer_header_bad_version_rejected() {
        let mut footer = empty_footer();
        footer.header.footer_version = 1;
        let bytes = footer.serialize();
        assert!(matches!(
            CoveFooter::parse(&bytes),
            Err(CoveError::BadVersion)
        ));
    }
    ...
    fn footer_header_metadata_too_large_rejected() {
        ...
        assert!(matches!(
            CoveFooterHeaderV1::parse(&bytes),
            Err(CoveError::BadSection(_))
        ));
    }
    ...
    fn footer_compute_crc_matches_serialized_bytes() {
        let mut footer = empty_footer();
        footer.metadata_json = br#"{"spec":"cove"}"#.to_vec();
    ...
    fn section_entry_rejects_unknown_section_kind() {
        let mut entry = CoveSectionEntryV1 {
            section_id: 1,
