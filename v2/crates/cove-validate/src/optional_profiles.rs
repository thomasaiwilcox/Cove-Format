use cove_core::{
    ...
    reader::{OptionalPushdownPolicy, ValidationReport},
    CoveError,
};
use cove_coverage::{
...
pub(crate) fn validate_embedded_optional_profile_sections(
    data: &[u8],
...
mod tests {
    ...
    fn entry(kind: SectionKind, required_features: u64) -> CoveSectionEntryV1 {
        CoveSectionEntryV1 {
            section_id: 7,
    ...
    #[test]
    fn fail_open_requires_policy_no_required_features_and_unrequested_section() {
        assert!(can_fail_open(
            &entry(SectionKind::LayoutPlan, 0),
        ...
        assert!(!can_fail_open(
            &entry(SectionKind::LayoutPlan, 1),
            OptionalPushdownPolicy::FailOpen,
            None
        ));
        let request = FeatureUseRequestV2::new().with_section(7);
        assert!(!can_fail_open(
            &entry(SectionKind::LayoutPlan, 0),
            OptionalPushdownPolicy::FailOpen,
            Some(&request)
    ...
    #[test]
    fn requested_operations_require_their_optional_payload_families() {
        let coverage = FeatureUseRequestV2::new().with_operation(OperationKindV2::CoveragePlanning);
        assert!(!can_fail_open(
            &entry(SectionKind::CoverageSet, 0),
            OptionalPushdownPolicy::FailOpen,
            Some(&coverage)
        ));
        let index_only =
            FeatureUseRequestV2::new().with_operation(OperationKindV2::IndexOnlyAnswer);
        assert!(!can_fail_open(
            &entry(SectionKind::IndexOnlyCapability, 0),
            OptionalPushdownPolicy::FailOpen,
            Some(&index_only)
        ));
        let zero_copy = FeatureUseRequestV2::new().with_operation(OperationKindV2::ZeroCopyExport);
        assert!(!can_fail_open(
            &entry(SectionKind::ZeroCopyBufferMap, 0),
            OptionalPushdownPolicy::FailOpen,
            Some(&zero_copy)
        ));
    }
    ...
    #[test]
    fn requested_profiles_require_their_optional_payload_families() {
        let layout = FeatureUseRequestV2::new().with_profile(PrimaryProfile::LayoutPlanning as u8);
        assert!(!can_fail_open(
            &entry(SectionKind::FastMetadataIndex, 0),
            OptionalPushdownPolicy::FailOpen,
            Some(&layout)
        ));
        let coverage =
            FeatureUseRequestV2::new().with_profile(PrimaryProfile::CoverageMetadata as u8);
        assert!(!can_fail_open(
            &entry(SectionKind::CoverageProofRecord, 0),
            OptionalPushdownPolicy::FailOpen,
            Some(&coverage)
        ));
        let secondary =
            FeatureUseRequestV2::new().with_profile(PrimaryProfile::SecondaryIndex as u8);
        assert!(!can_fail_open(
            &entry(SectionKind::IndexOnlyCapability, 0),
            OptionalPushdownPolicy::FailOpen,
            Some(&secondary)
